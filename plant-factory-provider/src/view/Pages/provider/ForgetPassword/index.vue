            ...
            <v-row>
              <v-col cols="6">
                <v-text-field
                  label="请输入验证码"
                  :rules="validatedrules"
                  v-model="validatedForm.validated"
                ></v-text-field>
              </v-col>
              <v-col></v-col>
              <v-col cols="5">
                <v-img width="100" height="50" img :src="img" alt="" v-if="showImg" @click="getImageagain"></v-img>
              </v-col>
            </v-row>
            <v-row>
...
export default {
  data() {
    return {
      ...
      surepasswordrule: [
        (v) => !!v || "确认密码不能为空",
        (v) => v === this.phoneForm.newpassword || "密码不一致",
        (v) => (v && v.length >= 8) || "确认密码须8位以上"
      ],
      ...
      if (valid) {
        getImage()
          .then((data) => {
            ...
            this.sessionKey = data.payload.sessionKey;
            this.validatedForm.validated = "";
            this.showImg = true;
            this.img = "data:image/png;base64," + data.payload.base64ImageCaptcha;
          })
          .catch((err) => {
    ...
    findpassword() {
      ...
      if (valid) {
        findpassword({
          ...
          smsCaptcha: this.phoneForm.validated
        })
          .then((data) => {
