  ...
  <v-app-bar app :src="src">
    ...
    <template v-slot:extension>
      <v-chip-group v-model="chipPath" active-class="red" @change="chipChange" mandatory>
        <v-chip v-for="item in chips" close label outlined small close-icon="mdi-close" :value="item.path" :key="item.path" @click:close="chipClose(item.path)">
          {{ item.title }}
        </v-chip>
      </v-chip-group>
    </template>
...
export default {
  ...
  methods: {
    ...
    myInfo() {
      ...
      // });
      this.$router.push("/admin/platform/platformuser");
    },
    // 初始化nav纸片导航route数据
    getRouteList() {
