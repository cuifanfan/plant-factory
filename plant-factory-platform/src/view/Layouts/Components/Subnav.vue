<template>
  <!-- 侧边导航栏组件 -->
  <v-navigation-drawer app color="#304156" class="evevation-6" :value="isopen" :mini-variant="ismini" @input="open($event)" mobile-breakpoint="1200">
    <template slot="prepend" class="pa-2">
      <div class="pa-1" style="overflow: hidden">
        <v-img src="../../../assets/images/logo.png" alt="方芯水培" class="rounded-lg float-left" height="60" width="60"></v-img>
        <div class="ml-15 logo" style="height: 60px;font">平台商管理</div>
      </div>
    </template>
      ...
      <template v-for="(itemGroup, i) in listGroups">
        <template v-if="itemGroup.children && itemGroup.children.length">
          <v-list-group :key="i" color="#1796f9" v-if="!itemGroup.meta.isHide">
            ...
            <v-list-item v-for="(item, j) in itemGroup.children" :key="j" :to="toPath(item.path)" link :class="ismini ? '' : 'pl-8'" class="blue-grey darken-4 bg" style="background-color: #304156">
              <v-list-item-icon>
                <v-icon
                  :style="{
