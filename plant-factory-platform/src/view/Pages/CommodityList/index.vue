<template>
  <v-card flat>
    ...
    <v-data-table v-model="selected" :search="search" :headers="headers" :items="desserts" :single-select="singleSelect" hide-default-footer loading="true" loading-text="未查询到数据">
    ...
    <v-dialog transition="dialog-bottom-transition" v-model="moreInfo" class="elevation-0 dialog" max-width="500" ref="dialog">
      <v-card class="mx-auto">
        <v-card-title>商品详情</v-card-title>
        <!-- 轮播图 -->
        <v-carousel height="300px" cycle interval="1500" delimiter-icon="mdi-minus" class="mb-5" hide-delimiter-background>
