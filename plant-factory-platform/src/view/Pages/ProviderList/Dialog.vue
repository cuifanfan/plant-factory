      ...
      <v-card-text class="py-0 pt-4">
        <v-form ref="form" v-model="valid" lazy-validation>
          <v-container>
            <v-row>
              <v-col class="py-1">
                <v-text-field label="用户名" outlined dense v-model="name" :rules="[(v) => !!v || '不能为空']"> </v-text-field>
              </v-col>
              <v-col class="py-1">
                <v-select
                  :items="[
            ...
            <v-row>
              <v-col class="py-1">
                <v-select :items="provinceItem" label="省份" dense v-model="provinceCode" @input="getCityItem($event)" clearable outlined :rules="[(v) => !!v || '必须选择']"></v-select>
              </v-col>
              <v-col class="py-1">
                <v-select :items="cityItem" label="城市" dense v-model="cityCode" clearable no-data-text="请先选择省份" outlined :rules="[(v) => !!v || '必须选择']"></v-select>
              </v-col>
            </v-row>
            <v-row>
              <v-col class="py-1">
                <v-text-field label="详细位置" outlined dense v-model="address" :rules="[(v) => !!v || '不能为空']"></v-text-field>
              </v-col>
            </v-row>
            ...
            <v-row>
              ...
              <v-col class="py-1">
                <v-text-field label="负责人手机号" outlined dense v-model="principalPhone" :rules="[(v) => !!v || '不能为空']"></v-text-field>
              </v-col>
            </v-row>
          </v-container>
        </v-form>
