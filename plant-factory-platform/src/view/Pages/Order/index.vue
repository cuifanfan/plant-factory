  ...
  <div>
    ...
    >
          ...
          <v-row class="py-5 px-7 pb-0" align="center">
            <my-search v-model="showSearch">
              <template slot="header">
                <v-text-field placeholder="收货人电话" label="收货人电话" style="max-width: 200px" dense outlined :rules="[rules.number]" v-model="searchForm.recphone"></v-text-field>
                <v-text-field placeholder="购买人电话" outlined label="购买人电话" style="max-width: 200px" :rules="[rules.number]" dense class="ml-3" v-model="searchForm.ownerphone"></v-text-field>

                <v-btn text class="light-blue align-self-start darken-1 ml-auto" color="white" depressed min-width="100" @click="searchInfo()"> 搜索 </v-btn>
                <v-btn text class="pa-1 ml-3 align-self-start" color="light-blue darken-1" @click="showSearch = !showSearch"
                  >高级搜索
              ...
              <v-list class="pa-0 mt-2">
                ...
                <v-list-item class="pa-0 mt-2" style="height: 60px">
                  <v-text-field placeholder="收货人姓名" outlined label="收货人姓名" :rules="[rules.name]" style="max-width: 200px" dense v-model="searchForm.rcvName"></v-text-field>
                </v-list-item>
              </v-list>
            </my-search>
          </v-row>
      ...
      <template slot="item.moreInfo" slot-scope="item">
        <v-btn min-width="90px" class="white--text text--lighten-1" color="#039be5" depressed small @click="searchsingle(item.item)"> 详情 </v-btn>
      </template>
    ...
    <v-dialog transition="dialog-bottom-transition" v-model="showDialog" class="elevation-0" max-width="550" persistent>
      <v-card class="mx-auto">
        <v-card-title style="position: fixed; z-index: 9999; width: 550px" class="white"
          ><span class="text-h5">订单详情</span>
          <v-spacer></v-spacer>
          <v-icon large @click="showDialog = false">mdi-close-octagon-outline</v-icon></v-card-title
        >
        <v-card-text class="pt-16">
          <div v-for="(value, key, index) in curritem" :key="index">
            <v-row v-if="key == 'status'">
              <v-col class="text-h6 textStyle" cols="6">{{ status[key] }}</v-col>
              <v-col cols="6"
                ><v-select :items="items" label="订单状态 " dense outlined :value="`${value}`" item-text="key" item-value="value" @input="curritem.status = $event" hide-details></v-select
              ></v-col>
            </v-row>
            <v-row v-else-if="key == 'type'">
              <v-col class="text-h6 textStyle" cols="6">{{ status[key] }}</v-col>
              <v-col cols="6">{{ orderType[value] }}</v-col>
            </v-row>
            <v-row v-else-if="key == 'items'" v-for="(item, index) in value" :key="index">
              <v-col class="text-h6 textStyle" cols="6"> 购买商品{{ index + 1 }} </v-col>
              <v-col cols="6">
            ...
            <v-row v-else>
              <v-col class="text-h6 textStyle" cols="6">{{ status[key] }}</v-col>
              <v-col cols="6" class="text-subtitle-1">{{ value }}</v-col>
            </v-row>
          </div>
        </v-card-text>
        ...
        <v-card-actions>
          <div class="mx-auto">
            <v-btn text style="border: 1px solid #eeeeee" depressed min-width="100" @click="showDialog = false"> 取消修改 </v-btn>
            <v-btn text class="light-blue darken-1 ml-3" color="white" depressed min-width="100" @click="save"> 修改订单 </v-btn>
          </div>
        </v-card-actions>
      </v-card>
    </v-dialog>
...
<script>
import { exportOrders, updateOrderStatus, searchAllorder } from "@/api/order";
import mySearch from "@/view/Components/Search.vue";
import myDate from "@/view/Components/Date.vue";
  ...
  data() {
    return {
      ...
      dessertHeaders: [
        ...
        { text: "收货人Id", value: "rcvInfo" },
        { text: "订单创建时间", value: "createTime" },
        { text: "订单发货时间", value: "sendTime" },
        { text: "订单关闭时间", value: "closeTime" },
        { text: "订单完成时间", value: "completeTime" },
        { text: "订单关闭原因", value: "closeReason" },
        { text: "订单备注", value: "remark" },
  ...
  methods: {
    ...
    getProviderOrderList() {
      searchAllorder({
        ...
        this.desserts.forEach((element) => {
          ...
          if (element.sendTime) {
            element.sendTime = element.sendTime.replace("T", " ");
          }
          if (element.closeTime) {
            element.closeTime = element.closeTime.replace("T", " ");
          }
        });
    ...
    save() {
      ...
      updateOrderStatus({
        ...
        statusCode: this.curritem.status
      }).then(
        (res) => {
          console.log(res);
    ...
    delitems(items) {
      for (let item in items) {
        if (this.purchaseItem.hasOwnProperty(item)) {
          if (item == "spec") {
            // items.item = items.item.replace('{','').replace('}','')
            items[item] = items[item].replace("{", "").replace("}", "");
            items[item] = items[item].replace(",", " ");
          }
          continue;
        } else {
          delete items[item];
