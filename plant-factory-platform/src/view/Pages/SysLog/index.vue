<template>
  <div>
    <v-data-table v-model="selected" :headers="headers" :items="desserts" class="" hide-default-footer loading="true" loading-text="未查询到数据" :items-per-page="10" no-data-text="没有数据">
      <template slot="top">
        <v-row class="py-5 px-7 pb-0" align="center">
          <my-search v-model="showSearch">
            <template slot="header">
              ...
              <v-text-field label="服务商" outlined style="max-width: 200px" dense class="ml-3" v-model="group"> </v-text-field>
            ...
            <v-list class="pa-0">
              ...
              <v-list-item class="pa-0 pt-1">
                <v-text-field label="操作的模块" outlined style="max-width: 200px" dense v-model="module"></v-text-field>
              </v-list-item>
            </v-list>
          </my-search>
        </v-row>
...
import { getLogList, exportPlatformLogs, getPlatformLogList } from "@/api/log";
export default {
  data: () => ({
    //进度条
    process: false,
