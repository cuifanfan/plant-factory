  ...
  <div>
    ...
    >
      ...
      <template slot="item.moreInfo" slot-scope="item">
        <v-btn min-width="90px" class="white--text text--lighten-1" color="#039be5" depressed small @click="searchsingle(item.item.id)"> 详情 </v-btn>
        <v-btn color="#98e165" class="ml-4 white--text success" depressed small @click="distributServer(item.item.id)"> 分配工程师 </v-btn>
      </template>
    ...
    <v-dialog transition="dialog-bottom-transition" v-model="distributeDialog" class="elevation-0" max-width="500">
      <v-card class="mx-auto">
        ...
        <v-card-text>
          <v-select :items="engineer" label="分配工程师" dense outlined item-text="name" item-value="phone" v-model="selectEnginer.engineerPhone"></v-select>
        </v-card-text>
        <v-card-actions>
