<template>
  <div class="ChangePassword">
    <v-dialog v-model="dialog" max-width="500px" no-click-animation transition="slide-y-transition" persistent>
      <v-card class="pb-4">
        <v-card-title>
          <span class="subtitle-1">修改密码</span>
        </v-card-title>
        <v-form class="ma-4 pa-4" ref="form" lazy-validation>
          <v-text-field
        ...
        <v-card-actions class="mx-4">
          ...
          <v-spacer></v-spacer>
          <v-btn @click="$emit('close', false)">取消</v-btn>
          <v-btn @click="validate">确认</v-btn>
        </v-card-actions>
      </v-card>
    </v-dialog>
  </div>
    ...
    return {
      pwdItems: [
        ...
        },
        {
          ...
          password: "",
          showRules: false,
          rules: [(v) => /^[A-Za-z][A-Za-z0-9!@#$%^&*._-]{7,19}$/.test(this.pwdItems[1].password) || "密码必须以字母开头"]
        },
        {
          tip: "确认新密码",
          show: false,
